...
%                             so a caller can check Vmat(3,3,:) == contract_cc(G4cc, Kmat).
...
%     COMPLETE HYBRID MAP (7D rework): the returned Vmat is then GENERATED BY the returned
...
% so a caller can verify Vmat(3,3,:) == contract_cc(G4cc, Kmat) EXACTLY (the returned
% Vcc is generated by the returned Kmat). G4cc is [] on the full-dress branch.

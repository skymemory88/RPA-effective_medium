...
%
%   opts.chi_base [3,3,nwn] (default [] = OFF): a FIXED local-response offset added to
%     the internally dressed chi BEFORE the EMT closure, so the medium/vertex fixed point
%     runs on (chi_dress + chi_base). Default OFF is BIT-IDENTICAL to the legacy loop. The
%     ordered a3d passes chi_base = chi_full - chi_dom to iterate the COMPLETE hybrid map
%     (7D rework); out.chi_til is then the hybrid chi_dress + chi_base.
%
%   OUTPUTS (struct out)
%     Vmat        [3,3,nwn] : the tensor self-energy state at which chi_til/Kmat
%                             were evaluated (V = G0.Sigma).
%     Vnext       [3,3,nwn] : the full map output regenerated from Kmat.
%     residual                 Vnext-Vmat, with infinity/scaled/component/frequency
%                             maxima and an explicit evaluation_kind.
%     chi_til     [3,3,nwn] : Dyson (symmetric-bracket) renormalized local chi (+ chi_base
%                             when a hybrid offset is supplied).
%     G4cc, Lmax           : compact cc;cc vertex + l-window (dominant path; G4cc [] else)
%                             so a caller can check Vnext(3,3,:) == contract_cc(G4cc, Kmat).
...
if ~(isscalar(maxo) && isfinite(maxo) && maxo == round(maxo) && maxo >= 1)
    error('invzt:maxOuter', 'opts.max_outer must be a positive integer; got %s.', mat2str(maxo));
end
nwn   = numel(wn);
Lmax  = getf(opts, 'Lmax', nwn - 1);
requested_evaluation = char(getf(opts, 'evaluation_kind', 'solve'));
if ~ismember(requested_evaluation, {'solve', 'one_step_reevaluation'})
    error('invzt:evaluationKind', ['opts.evaluation_kind must be ''solve'' or ' ...
        '''one_step_reevaluation''; got ''%s''.'], requested_evaluation);
end
...
max_states = NaN;  max_work = NaN;  max_bytes = NaN;  tile_nl = NaN;
work = NaN;  est_bytes = NaN;  Nd = N;
if dom
    ...
    Nd = numel(Ed);
    requested_rank = getf(opts, 'requested_vertex_rank', Nd);
    if ~(isscalar(requested_rank) && requested_rank == round(requested_rank) ...
            && requested_rank == Nd)
        error('invzt:vertexRankMismatch', ['requested vertex rank must equal the ' ...
            'realized dominant-basis rank (%d); got %s.'], Nd, mat2str(requested_rank));
    end
    if Nd > max_states
...
% --- opts.chi_base: a FIXED [3,3,nwn] local-response offset ADDED to the internally
%     dressed response BEFORE the EMT closure, so the medium K -- and the vertex it
%     feeds -- sees (chi_dress + chi_base), not chi_dress alone. Default OFF (empty) ->
%     BIT-IDENTICAL legacy behavior (the loop keeps chi_til = resum_dyson(...) verbatim).
%     The ordered a3d passes chi_base = chi_full - chi_dom so this V-loop iterates on the
%     COMPLETE HYBRID MAP (7D rework): the returned Vnext is then GENERATED BY the returned
%     Kmat = emt(chi_dress + chi_base) -- the omitted (n_full - n_vertex) spectator/ODD
%     response feeds back at the SAME vertex order, not as an O(1/z^2) afterthought.
chi_base = getf(opts, 'chi_base', []);
add_base = ~isempty(chi_base);
if add_base && ~isequal(size(chi_base), [3 3 nwn])
    error('invzt:chiBase', 'opts.chi_base must be [3,3,%d]; got %s.', nwn, mat2str(size(chi_base)));
end

% --- (1)-(3) outer self-consistent loop on Vmat ----------------------------------
Vmat = complex(zeros(3, 3, nwn));
if isfield(opts, 'Vmat_seed') && ~isempty(opts.Vmat_seed)
    if ~isequal(size(opts.Vmat_seed), [3 3 nwn]) || any(~isfinite(opts.Vmat_seed(:)))
        error('invzt:VmatSeed', 'opts.Vmat_seed must be a finite [3,3,%d] array.', nwn);
    end
    Vmat = opts.Vmat_seed;
end
converged = false;
...
for outer = 1:maxo
    ...
    if add_base, chi_til = chi_til + chi_base; end              % COMPLETE-map hybrid offset
    ...
    if outer == maxo, break; end                                  % retain evaluated co-state
    fv = f(:);  xv = Vnew(:);
...
% Vmat is deliberately the LAST EVALUATED INPUT, while Vnew is its full map output.
% Thus chi_til/Kmat/chi_bar are dressed from the reported Vmat and Vnew is regenerated
% from that same medium on every exit, including a capped or diagnostic one-step exit.
f = Vnew - Vmat;
dV = max(abs(f(:)));
residual_scale = max([1; abs(Vmat(:)); abs(Vnew(:))]);
residual_scaled = dV / residual_scale;
residual_component_max = max(abs(f), [], 3);
residual_frequency_max = reshape(max(max(abs(f), [], 1), [], 2), [], 1);
if strcmp(requested_evaluation, 'one_step_reevaluation')
    evaluation_kind = 'one_step_reevaluation';
elseif converged
    evaluation_kind = 'converged_solve';
else
    evaluation_kind = 'rejected_capped_solve';
end

chi_til_add = -(G0 - Vmat);                                    % additive resummation
if add_base, chi_til_add = chi_til_add + chi_base; end         % same hybrid offset (diagnostic)
Gloc = -real(squeeze(chi_bar(3, 3, :)));

out = struct();
out.Vmat = Vmat;  out.chi_til = chi_til;  out.chi_til_add = chi_til_add;
out.Vnext = Vnew;  out.residual = f;
out.residual_inf = dV;  out.residual_scaled = residual_scaled;
out.residual_component_max = residual_component_max;
out.residual_frequency_max = residual_frequency_max;
out.residual_tol = tolo;  out.evaluation_kind = evaluation_kind;
out.terminal_co_state_consistent = true;
...
out.vertex = struct('requested_rank', requested_rank, 'realized_rank', Nd, ...
    'max_vertex_states', max_states, 'max_vertex_work', max_work, ...
    'max_vertex_bytes', max_bytes, 'estimated_work', work, ...
    'estimated_peak_bytes', est_bytes, 'tile_nl', tile_nl, ...
    'nwn', nwn, 'nl', nl, 'Lmax', Lmax, ...
    'cache_policy', 'persistent_six_entry_legacy');
% Self-generation diagnostics (7D): the compact cc;cc vertex Gamma16 and its l-window,
% so a caller can verify Vnext(3,3,:) == contract_cc(G4cc, Kmat) EXACTLY. G4cc is
% [] on the full-dress branch.
out.G4cc = G4cc;  out.Lmax = Lmax;
end
...
CACHE(end+1) = struct('key', key, 'sig', sig, 'G4', G4);
% One full entry stores 9*3*3 complex [nwn,nl] pages: about 1.32 GiB at
% nwn=740, nl=1479. This is not the compact cache's approximately 16.7 MiB.
if numel(CACHE) > 6, CACHE(1) = []; end                    % bounded to six full entries
end
...
CACHE(end+1) = struct('key', key, 'sig', sig, 'G4cc', G4cc);

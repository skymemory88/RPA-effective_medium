...
```
Expected: `Bc ≈ [2.148 1.916]` (warm-cache knowns), both `TcB` finite with `Tc(0.5 T)` > `Tc(1.5 T)` and `Tc(0.5 T)` at/below the proxy Tc0 ≈ 1.53. NB execution finding E2 (spec rev 5): in the shallow-crit odd-on corner the T-cut and field-cut scan paths disagree by ~50 mK at this coarse grid — measured `Tc(1.5 T)` ≈ 1.35–1.36 K (NOT the naive 1.42–1.50 band inferred from the field cut; the gates below assert only ordering and the proxy cap, which E2 respects). Finite proxy + comparator, `SMOKE OK`. Estimated ~10–25 min serial, warm caches (estimate, not a gate). The T-cut success with `odd = true` (the driver default) is the two-branch demonstration; if either `TcB` is NaN, STOP and report with the console output — do not widen windows or toggle `odd`.

...
largest_nonfinite = NaN;
if any(positive & ~current_finite)
    largest_nonfinite = max(M2(positive & ~current_finite));
end
result = struct('table',tab,'healthy_fixture_M2',pt.tl.M2, ...
    ...
    'largest_nonfinite_M2',largest_nonfinite, ...
    ...
    'note',['At M2=0 production evaluates the exact reassociation ' ...
    'Q=2m^2[lambda1-(1-n01^2)K0]/n01^2. Positive-M2 states retain the ' ...
    'historical arithmetic bit-for-bit. The earlier measured-node census ' ...
    'remains valid that small-M2 arithmetic did not cause the sampled masks; ' ...
    'the exact-zero endpoint independently supplied the reason to define ' ...
    'the removable endpoint.']);
...
fprintf(['legacy direct ratio would overflow below M2 %.3g; production ' ...
    'largest sampled nonfinite M2 %.3g; M2=0 ||Sigma|| %.6g\n'], ...

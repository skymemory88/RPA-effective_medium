...
auditFormulation = getf(opts,'audit_formulation','nested');
if isstring(auditFormulation)
    if ~isscalar(auditFormulation) || ismissing(auditFormulation)
        error('invzp:OrderedArc:InvalidInput', ...
            'opts.audit_formulation must be ''nested'' or ''coupled''.');
    end
    auditFormulation = char(auditFormulation);
end
if ~(ischar(auditFormulation) && isrow(auditFormulation) && ...
        any(strcmp(auditFormulation,{'nested','coupled'})))
    error('invzp:OrderedArc:InvalidInput', ...
        'opts.audit_formulation must be ''nested'' or ''coupled''.');
end
if strcmp(auditFormulation,'coupled') && ...
        ~strcmp(getf(ctx.eso,'audit_coordinate','raw_closure'),'defactored')
    error('invzp:OrderedArc:InvalidInput', ...
        ['opts.audit_formulation=''coupled'' requires ', ...
         'ctx.eso.audit_coordinate=''defactored''.']);
end
...
cfg = struct('schema','invzp_ordered_arclength_problem/v1', ...
    ...
    'tol_outer',tolOuter,'audit_formulation',auditFormulation, ...
...
problem = struct( ...
    ...
    function [R,Jscaled,record] = equations(y)
        ...
        [R,physics,state,Ju,factors] = ...
            invz_ordered_node_equations(node,u);

        dh = max(hFdAbsolute,hFdRelative*max(hScale,abs(h)));
        [RhCoarse,statusCoarse,whCoarse] = hDerivative(u,h,dh);
        [RhHalf,statusHalf,whHalf] = hDerivative(u,h,dh/2);
        Rh = (4*RhHalf-RhCoarse)/3;
        wh = (4*whHalf-whCoarse)/3;
        derivativeDrift = norm(Rh-RhHalf,Inf)/max(1,norm(Rh,Inf));
        wDerivativeDrift = abs(wh-whHalf)/max(1,abs(wh));
        Jscaled = [Ju.*stateScale.',Rh*hScale];
        wGradientScaled = [factors.w_gradient.*stateScale.',wh*hScale];
        ...
        record = struct('h',h,'u',u,'node',node,'state',state, ...
            ...
            'w',factors.w, ...
            'w_gradient_scaled',wGradientScaled, ...
            'w_h_derivative',wh, ...
            'w_h_scaled_drift',wDerivativeDrift, ...
            'h_derivative_status',derivativeStatus);
        updateCache(y,R,Jscaled,record);
    end

    function [Rh,status,wh] = hDerivative(u,h,dh)
        ...
        [Rplus,plus] = invz_ordered_node_equations(nodePlus,u);
        [Rminus,minus] = invz_ordered_node_equations(nodeMinus,u);
        Rh = (Rplus-Rminus)/(2*dh);
        wh = (plus.z-minus.z)/(2*dh);
        status = 'ok';
    end
    ...
    function [accepted,reason,payload] = audit(~,record)
        auditRecord = invz_ordered_residual( ...
            record.node,record.state,struct( ...
            'tol_outer',tolOuter,'formulation',auditFormulation));
        ...
        payload = struct('h',record.h,'state',record.state, ...
            ...
            'w',record.w, ...
            'w_gradient_scaled',record.w_gradient_scaled, ...
            'w_h_derivative',record.w_h_derivative, ...
            'w_h_scaled_drift',record.w_h_scaled_drift, ...
            'audit',auditRecord);
    end
    ...
    function record = invalidRecord(h,u,local,derivativeStatus)
        ...
        record = struct('h',h,'u',u,'node',[],'state',state, ...
            ...
            'w',NaN,'w_gradient_scaled',nan(1,nw+2), ...
            'w_h_derivative',NaN,'w_h_scaled_drift',NaN, ...
            'h_derivative_status',derivativeStatus);
    end

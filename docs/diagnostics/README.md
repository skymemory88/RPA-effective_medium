...
| `biased_smooth_r_2026-07-28/` | isolated oracles for the documented non-default smooth-`r(h)` route: a pure selector, generic bordered pseudo-arclength tracer, resummed invZ adapter, explicit-seed root enumerator, strict fixed-`h` trace-to-graph input adapter, and evidence-only graph assembler. The retained traces reproduce multiple 1.5 T folds with full states and A--D audits, but no continuous signed-edge certifier, complete single-valued Jensen-section builder, endpoint solver, or production dispatch exists |

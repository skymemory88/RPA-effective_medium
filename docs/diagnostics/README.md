...
| `biased_smooth_r_2026-07-28/` | isolated oracles for the documented non-default smooth-`r(h)` route: a pure selector, generic bordered pseudo-arclength tracer, and resummed invZ adapter. The retained tracer reproduces the 1.5 T high-branch fold with full states and A--D audits, but no global root/path enumerator, endpoint solver, or production dispatch exists |

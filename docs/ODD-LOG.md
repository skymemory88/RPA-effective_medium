...
## A2 — Matrix effective medium (T9) (2026-07-18)

Plan v3. Commit 1239c33. Tensor CORE 26/26 (invz_projected off-path).
...
## A-ordered — Ordered-side (FM) a1 + dominant-vertex a3d (2026-07-19/20)

Plan: docs/superpowers/plans/2026-07-19-invzt-ordered-side.md. Commits c0c723e (T1) / 7834593 (T2) / ef54a52 (T3) / 456c924 (T4) / e584555 (T5) / d2c4bae (T6) / 500033a (7A) / 67054f1+b5305ee (7B) / e5e3e22 (7C) / a25a6cf+886d105+1a94f51 (7D + rework + fix). Two Codex review rounds (P0-1..P0-3, P1-1..P1-6, P2-1..P2-2) dispositioned pre-execution; every finding CONFIRMED on re-verification (docs/superpowers/plans/2026-07-19-invzt-ordered-side.md, both disposition tables).

**Implemented (T1-T7D):**
- T1: `invz_sigma_ordered` / `invz_twolevel_ordered` moved projected -> invz_common (git mv, behavior-neutral); invz_projected/README.html updated (3 references).
- T2: `invzt_crit_static` extracted from `invzt_solve_point`'s `local_crit` (bit-identical), shared by the PM and ordered a1 solvers and by a3d.
- T3: `invzt_solve_point_ordered` — tensor a1 FM point solver (moment-form Sigma, invz_sigma_ordered, on the tensor lattice engine). AMENDED mid-execution to a WHOLE-CC medium (no dominant/rest split): the `E < Esplit` cut carried only 47.6% of the cc weight at 0.1 K ordered points, making the parity gate structurally unsatisfiable (see headline numbers). `Esplit`/`chi_rest` now error `invzt:orderedSplitKnobs` on this solver.
- T4: ordered branch in `invzt_chi_realaxis`, gated by the EXACT n=0 formula identity (frozen-Kw `gamma_w == gamma0`); real-axis for a3d stays OPEN (Matsubara-only, see 7D).
- T5: `invzt_solve_auto` — STABILITY-BASED phase dispatcher (review P0-1 redesign): the PM leg's `crit > 0` decides the phase FIRST (the bare-MF ordered moment persists well past the true QPT — ordered-first would misassign the boundary); ordered leg consulted only when PM is invalid; entry-validated (Bz + mode) before either leg; structured per-leg diagnostics `di.para`/`di.ordered` (review P2-1).
- T6: `invzt_run_spectra` rewired to SPAN Bc via `invzt_solve_auto`; three 2026-07-19 knob fixes (window, eta, peak_wmin — numbers below).
- T7A: `eps_el` fix (elastic-only `c_d`, constraint 7); two scalar-beta ordered oracle systems (`jensen_2lvl_ordered_b14`/`_b3`) in `verify_tensor_vertex.py`; ordered V-row test; static-gated (n=0) Jensen moment-form diagnostic, n>0/betaDelta=3 REPORT-only.
- T7B: fixed dominant vertex basis for a3d. Round-2's proposed fixed zero-field `e2xI8` content basis and the moving `E<Esplit` energy cut were BOTH measured and falsified (scorecard below); shipped basis is fixed-RANK (16), FIELD-ADAPTED (`invzt_ordered_vertex_basis`), with p_mass/Mx/My/isolation/var_share refinement diagnostics.
- T7C: compact `cc;cc`-only dominant vertex (no 1.42 GB zero-pad), l-tiling, `invzt:orderedA3Budget` guards; N=8/10/13/16 benchmark + Vcc(16)-vs-(24) convergence study.
- T7D: `mode='a3d'` wired into `invzt_solve_point_ordered` (full-response, fixed-rank field-adapted dominant-vertex hybrid). REWORKED mid-execution (user-required) onto the COMPLETE hybrid map after the as-built isolated-16-state fixed point was rejected (numbers below); production stub fixed (commit 1a94f51) to register as CORE Incomplete via `assumeTrue`, the repo's slow-gate convention.

**Suite status:** Tensor CORE progressed 63/0/4 (T3) -> 67/0/4 (T4) -> 75/0/4 (T5; T6 modifies only the driver script, no new CORE tests) -> 78/0/4 (7A) -> 81/0/4 (7B) -> 89/0/4 (7C) -> **96/0/4 PRE-FIX** (7D). The fix commit (1a94f51) makes the a3d production stub (`test_a3d_production_point`) register as Incomplete via `assumeTrue` rather than run/skip silently — the CORE Incomplete allowlist is therefore **5 names**: `test_ladder_production_slow`, `test_tcut_matches_field_cut_slow`, `test_tcut_odd_on_slow`, `test_tcut_adaptive_slide_slow` (the pre-existing 4-name baseline), plus `test_a3d_production_point`. PROJECTED suite (143/0/19) UNTOUCHED throughout — the ordered-side plan never modifies `invz_projected/` beyond the T1 README pointer update.

**Headline numbers:**

*a1 whole-cc anchors* (0.1 K, 3 T, 16^3 halfopen/dpRng-30, odd on): m0 = 4.2275, Sigma0 = -0.09508, crit = +0.66307, alpha_m = 0.06059, sumrule = 0.0477. Interop parity vs. the projected ordered solver (grid-matched, REPORT — a convention difference, not a defect): dm0 = 0, dSigma0 = 2.7e-4, dalpha_m = -2.2e-3, dcrit = 3.9e-2. (T3 execution finding motivating the whole-cc amendment: the `E<Esplit` dominant/rest split carried only 47.6% of the cc weight at these points, with dSigma0 = 8.6e-3 / dalpha_m = 5.8e-2 against the SAME parity target while `chi0cc0` stayed bit-identical and dm0 = 0 — a pure medium-convention divergence, not a physics bug.)

*Real-axis ordered branch* (T4): FM soft mode E(3.5 T) = 0.3422 > E(4.4 T) = 0.3078 meV (softens toward Bc); min chi'' = 5.6e-15; exact-formula gate max|ordered-PM| = 0.159.

*Stability dispatcher boundary scan* (T5): B = [4.55 .. 4.85] T -> phase = [1 1 1 2 2 2 2]; the flip lands inside the independently-measured 4.65-4.70 T crit bracket, monotone, zero masked.

*Ordered static gate + eps_el correction* (T7A): n=0 static Jensen-bridge gate at betaDelta=14: res = 1.07e-4 < tol0 = 3.10e-4 (shape separation from the n=1 residual ~1600x); betaDelta=3 REPORT-only res ~ 1.1 (elastic-sector resummation ambiguity, a3d input, constraint 7). **`eps_el` correction (constraint 7, review P1-4):** `pt.eps_el` now uses the ELASTIC-ONLY static cc weight `c_d` (elastic-on/off difference of the same chi0 convention the solve consumes) instead of the full equal-time variance `si.JzJz_fluct`. **Every `eps_el` value logged before this fix (all prior A0-A4 entries and any earlier a3 runs) is an UPPER BOUND, not the corrected quantity.**

*Acceptance sweep* (T6, 101 fields, T = 0.1 K): 42 ordered / 59 PM / 0 masked; phase boundary between 4.64-4.68 T (inside the 4.65-4.70 T crit bracket); Epeak V-shaped across the transition: 0.3478 meV (3 T) -> minimum 0.2447 meV (near 4.88 T) -> 0.4156 meV (7 T); the Option-A masked window measured EMPTY at the driver's 0.04 T field resolution.

*Vertex-basis falsification + scorecard* (7B): the fixed **zero-field** `e2xI8` content basis measured chi_share ~1e-13 (range 0.00000-0.00013 across 3.0-4.65 T, overlap_deficit 1.56-1.87) — FALSIFIED: the zero-field Ising doublet has <+|Jz|-> = 0, so it sees essentially none of the field-polarized cc response. The moving `E<Esplit` energy cut is ALSO falsified by count-drift: ndom 13->8 across 3.0-4.8 T. Shipped basis — fixed-RANK (16), FIELD-ADAPTED (16 lowest eigenstates of the ordered MF Hamiltonian, rebuilt every field): chi_share 0.977-0.994, p_mass 1.000000, gap_ratio 2.71-3.66, adjacent-field subspace overlaps >= 0.9993. **Scorecard: Esplit-cut 48% / zero-field-content 0% / fixed-rank-adapted 97.7%.** **var_share CAVEAT:** 0.6650 @ 3.0 T, 0.8381 @ 4.65 T — a high static chi_share (~0.98) does NOT prove vertex convergence (high-energy states are denominator-suppressed statically but re-enter the four-point vertex as virtual intermediates with no such suppression); the actual vertex-convergence check is the 7C Vcc(16)-vs-(24) study below.

*7C benchmark + convergence*: N = [8 10 13 16] build times [106.3 258.0 734.0 1687.0] s, fit p = 3.987 (~N^4); rank-16 production ONE-BUILD = **7.43 h <= 12 h HARD GATE PASS** (N=24 -> 37.6 h, N=32 -> 119 h, both budget-refused). CAVEAT: the fit is anchored on N<=16 — the measured Nv=24 build ran ~2.6x slower than the fit predicts (per-tile temporaries falling off a cache level), so N>16 hour projections are optimistic; the rank-16 production number is in the measured regime and unaffected. Convergence: Vcc(16)-vs-(24) relative spread = 9.867e-3 @ 0.1 K/3 T, 2.444e-2 @ 1.0 K/3 T -> **RANK 16 SUPPORTED** (few-percent at both anchors; 0.1 K spread < 1 K spread, gap_ratio strengthens at lower T).

*7D complete-hybrid-map rework* (user-required): the as-built isolated-16-state fixed point was REJECTED — its returned `Vcc` was NOT generated by its returned `Kmat` (invariant violation), and the omitted 119-state medium feedback is the SAME vertex order (var_share = 0.665 at 3 T says it is not negligible). Required rework: iterate V on the COMPLETE hybrid map (dress -> chi_H = chi_full + (chi_dom_til - chi_dom) -> EMT -> contract), via a `chi_base` seam on `invzt_sigma_tensor` (default 0 = bit-identical legacy, verified). Hard gates: self-generation residual RED **1.503 -> 0.000e+00**; complete-map reeval 1e-9; reduction identity 0.0. Isolated-vs-complete comparison: **dVcc(0) = 28.5%** (medium feedback is same-order — the rejection is VINDICATED, not merely cautious), dcrit = -0.014. a3d vs. a1 at 1 K/3 T: crit 0.28452 vs. 0.22358 (same sign, in gate; anchor moved 2T->3T because of an a1 Picard oscillation at 2 T, documented separately, not an a3d defect).

**Grid conv + dpRng provenance:** production numbers (T3-T6 CORE anchors/acceptance sweep, the 7D production point) use the 16^3 halfopen grid at dpRng-30 (T = 0.1 K anchor, matching the A1/A4 PM-side production convention). CORE/affordable-anchor structural gates (7A-7D) use the 8^3 halfopen grid at dpRng-15; the T3 INTEROP ordered-parity test (`test_invzt_solve_ordered_parity`) uses the 8^3 legacy_inclusive grid at dpRng-15, matching the pre-existing grid-contract convention (README &sect;7 "Suite topology" — legacy_inclusive is reserved for parity with projected/legacy anchors, never mixed with halfopen inside one comparison).

**Production a3d point:** 0.1 K / 3 T / mode 'a3d' on the 16^3/dpRng-30 grid — measured build time 7.4 h. User-launched `nohup` ONLY; never run in CORE or an interactive/CI harness (the harness silently kills long MATLAB runs). See `invz_tensor/SESSION-2026-07-19-invzt-ordered-side.md` for the launch command.

**Minors (deferred to final review):** `hash_vec`+C2-assert duplicated between the PM and ordered solvers; `rank_tol`->`invzt_crit_static` scoping asymmetry vs. the PM solver; `var_full` recomputed vs. reusing `si_full.JzJz_fluct` (7B); `rank_tol` 3-arg vs. 2-arg call in the crit consistency test (plan-conformant, 7D); a 173 s vertex build lands in CORE (cached after first run, 7D); a dead `sfloor` local at `invzt_run_spectra.m:118` with a stale "single-sourced" comment (T6 review finding).

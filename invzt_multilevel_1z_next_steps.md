...
protected_branch: invzp-exec-convg-plan
protected_branch_policy: read_only
...
1. **Branch isolation.** No write to the projected-spin worktree or branch.
...
1. branch is `invzt-multilevel-1z` and worktree is clean at the documentation
   checkpoint;
2. the projected worktree's status digest matches the journal protection audit;
